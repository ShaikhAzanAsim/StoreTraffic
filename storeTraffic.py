#!/usr/bin/env python3
"""
store_roi_people_count_resilient.py

Resilient version:
 - Tries several tracker options (bytetrack.yaml -> bytetrack -> no tracker)
 - Creates output writer only after first processed frame (avoids empty/corrupt files)
 - Removes output file if no frames processed
 - Simple config variables at top (no argparse)

Change: uses bottom-center point of bbox (feet area) for ROI membership checks.
"""
...
print(" - 'u' undo last point, 'r' remove last ROI, 's' start inference, 'e' quit")
...
try:
    for result in results_stream:
        ...
        # draw ROIs and overlays
        for ridx, poly in enumerate(rois):
            pts = np.array(poly, np.int32).reshape((-1,1,2))
            cv2.polylines(frame, [pts], isClosed=True, color=(0,200,255), thickness=2)
            cumul = len(roi_seen_ids[ridx]); current = len(roi_current_ids[ridx])
            label = f"ROI_{ridx+1} Cur:{current} Total:{cumul}"
            lx, ly = poly[0]
...
for ridx in range(len(rois)):
    print(f"ROI_{ridx+1}: unique_persons = {len(roi_seen_ids[ridx])}")
